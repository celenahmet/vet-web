  ...
  return (
    <div className="min-h-screen pt-32 pb-20 relative bg-[var(--bg-main)] flex flex-col justify-center">
      ...
      <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16">
        ...
        {/* 3 Packages Grid - Using items-start so cards don\'t stretch vertically to equal heights */}
        <div className="grid lg:grid-cols-3 gap-8 relative z-10 mb-20 items-start">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h3 className="text-3xl font-extrabold text-[var(--text-main)]">{t('pricing_tier1_name')}</h3>
              <PawPrint className="text-emerald-500" size={28} />
            </div>
            <p className="text-[var(--text-muted)] text-[15px] mb-8 min-h-[60px] leading-relaxed">
              {t('pricing_tier1_desc')}
            </p>
            <div className="mb-8 flex flex-col items-start gap-2 bg-[var(--color-vet-accent-light)] p-5 rounded-2xl border border-[var(--color-vet-primary)]/20 shadow-sm">
              <span className="text-[13px] font-bold text-[var(--color-vet-accent)] uppercase tracking-wide leading-snug">{t('pricing_tier1_badge')}</span>
            </div>

            <ul className="flex-1 text-[14px] font-medium mb-8">
            ...
            <Link to="/contact" className="mt-4 w-full py-4 rounded-xl font-bold btn-primary shadow-md hover:shadow-lg transition-shadow text-center text-[15px]">
              {t('pricing_btn_start')}
            </Link>
          </motion.div>
          ...
          >
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--color-vet-primary)] text-white px-6 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase shadow-md">
              POPÜLER
            </div>
            <div className="flex items-center gap-3 mb-4 mt-2">
              <h3 className="text-3xl font-extrabold text-[var(--text-main)]">{t('pricing_tier2_name')}</h3>
              <Star className="text-[var(--color-vet-primary)] fill-[var(--color-vet-primary)]" size={28} />
            </div>
            <p className="text-[var(--text-muted)] text-[15px] mb-8 min-h-[60px] leading-relaxed">
              {t('pricing_tier2_desc')}
            </p>
            <div className="mb-8 flex flex-col items-start gap-2 bg-[var(--color-vet-accent-light)] p-5 rounded-2xl border border-[var(--color-vet-primary)]/20 shadow-sm">
              <span className="text-[13px] font-bold text-[var(--color-vet-accent)] uppercase tracking-wide leading-snug">{t('pricing_tier2_badge')}</span>
            </div>
            ...
            <Link to="/contact" className="mt-4 w-full py-4 rounded-xl font-bold btn-primary shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all text-center text-[15px]">
              {t('pricing_btn_start')}
            </Link>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h3 className="text-3xl font-extrabold text-[var(--text-main)]">{t('pricing_tier3_name')}</h3>
              <Building2 className="text-slate-700 dark:text-slate-300" size={28} />
            </div>
            <p className="text-[var(--text-muted)] text-[15px] mb-8 min-h-[60px] leading-relaxed">
              {t('pricing_tier3_desc')}
            </p>
            <div className="mb-8 flex flex-col items-start gap-2 bg-slate-100 dark:bg-slate-800/80 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
              <span className="text-[14px] font-extrabold text-slate-800 dark:text-slate-200 uppercase tracking-widest leading-snug">{t('pricing_tier3_badge')}</span>
            </div>
            ...
            <Link to="/contact" className="mt-4 w-full py-4 rounded-xl font-bold bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-md hover:shadow-lg transition-shadow text-center flex items-center justify-center gap-2 text-[15px]">
...
console.log('Pricing.tsx rewritten with custom heights, borders, icons, and more spacing.');
