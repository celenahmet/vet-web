      ...
      <div className="w-full relative bg-[var(--bg-main)]">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center gap-8 lg:gap-12 w-full">
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="flex flex-col items-start z-10 w-full lg:w-1/2">
                <motion.div variants={fadeInUp} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-indigo-200 dark:border-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-sm font-bold shadow-sm mb-6 bg-indigo-50/50 dark:bg-indigo-900/20 backdrop-blur-sm">
                  <Sparkles size={16} />
                  Hayvanseverlerin ve Kliniklerin Yeni Ortak Noktası
                </motion.div>
                
                <motion.h1 variants={fadeInUp} className="font-extrabold leading-[1.1] tracking-tight mb-6 text-[var(--text-main)]">
                  <span className="text-4xl lg:text-5xl leading-tight">Can Dostlarımızın Sağlık Yönetiminde <br/></span>
                  <span className="text-5xl lg:text-6xl bg-clip-text text-transparent bg-gradient-to-r from-[#038d91] to-[#33c8c2]">Yeni Bir Dönem</span> 
                </motion.h1>
                
                <motion.p variants={fadeInUp} className="text-lg md:text-xl text-[var(--text-muted)] mb-10 max-w-xl leading-relaxed font-medium">
                  Bilinçli hayvan sahipleri için akıllı hatırlatıcılar, dijital karneler ve sosyal topluluk... Modern veteriner klinikleri içinse evrak yükünü tamamen ortadan kaldıran, kusursuz bir randevu ve süreç yönetim sistemi. Veterito ile geleceğe adım atın!
                </motion.p>
        ...
        <section className="min-h-screen h-auto lg:h-screen snap-always snap-start py-24 lg:py-0 flex flex-col justify-center bg-[var(--bg-main)]">
          <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16">
            <div className="text-center max-w-2xl lg:max-w-3xl mx-auto mb-16">
              <h2 className="text-4xl lg:text-5xl font-extrabold text-[var(--text-main)] mb-6">Neden Veterito'ya Katılmalısınız?</h2>
              <p className="text-lg text-[var(--text-muted)] font-medium leading-relaxed">
                İster evcil hayvanınızın sağlığını yakından takip eden bir hayvansever olun, <br className="hidden md:block" />
                ister iş akışını dijitalleştiren modern bir klinik... Veterito, can dostlarımız için kurulan en kapsamlı, güvenilir ve yenilikçi dijital sağlık ağıdır!
              </p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{once:true}} className="bg-white dark:bg-slate-800 border-2 border-indigo-100 dark:border-indigo-900 rounded-3xl p-8 shadow-xl relative overflow-hidden group hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
                ...
                <h3 className="text-xl font-bold text-[var(--text-main)] mb-6">Her Şey Tek Bir Yerde</h3>
                <ul className="space-y-4">
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <User className="text-indigo-500 shrink-0 mt-0.5" size={18} /> <span>Evcil hayvanınızın tüm sağlık geçmişi ve karnesi cebinizde</span>
                  </li>
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <Stethoscope className="text-indigo-500 shrink-0 mt-0.5" size={18} /> <span>Klinik randevu ve operasyon süreçlerinizi tek ekrandan hızla yönetin</span>
                  </li>
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <CheckCircle className="text-indigo-500 shrink-0 mt-0.5" size={18} /> <span>Kaybolan evraklara ve karmaşık dosyalara dijital çözüm</span>
                  </li>
                </ul>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{once:true}} transition={{delay: 0.1}} className="bg-white dark:bg-slate-800 border-2 border-rose-100 dark:border-rose-900 rounded-3xl p-8 shadow-xl relative overflow-hidden group hover:border-rose-300 dark:hover:border-rose-700 transition-colors">
                <div className="w-12 h-12 bg-rose-50 dark:bg-rose-900/50 text-rose-500 rounded-xl flex items-center justify-center mb-6">
                  <Heart size={24} />
                </div>
                <h3 className="text-xl font-bold text-[var(--text-main)] mb-6">Kesintisiz İletişim</h3>
                <ul className="space-y-4">
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <User className="text-rose-500 shrink-0 mt-0.5" size={18} /> <span>Yaklaşan aşı ve periyodik bakım hatırlatmalarını asla kaçırmayın</span>
                  </li>
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <Stethoscope className="text-rose-500 shrink-0 mt-0.5" size={18} /> <span>Otomatik bildirimlerle hasta sahipleriyle güçlü bir bağ kurun</span>
                  </li>
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <CheckCircle className="text-rose-500 shrink-0 mt-0.5" size={18} /> <span>Aktif pati topluluğumuzla etkileşime geçip deneyimlerinizi paylaşın</span>
                  </li>
                </ul>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{once:true}} transition={{delay: 0.2}} className="bg-white dark:bg-slate-800 border-2 border-sky-100 dark:border-sky-900 rounded-3xl p-8 shadow-xl relative overflow-hidden group hover:border-sky-300 dark:hover:border-sky-700 transition-colors">
                <div className="w-12 h-12 bg-sky-50 dark:bg-sky-900/50 text-sky-500 rounded-xl flex items-center justify-center mb-6">
                  <Search size={24} />
                </div>
                <h3 className="text-xl font-bold text-[var(--text-main)] mb-6">Güvenli ve Erişilebilir</h3>
                <ul className="space-y-4">
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <CheckCircle className="text-sky-500 shrink-0 mt-0.5" size={18} /> <span>Tüm sağlık verileriniz ve test sonuçlarınız bulut tabanlı sistemimizde güvende</span>
                  </li>
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <CheckCircle className="text-sky-500 shrink-0 mt-0.5" size={18} /> <span>Veri kaybı, yıpranan karneler veya silinen tahliller riskini ortadan kaldırın</span>
                  </li>
                  <li className="flex items-start gap-3 text-[var(--text-muted)] font-medium">
                    <CheckCircle className="text-sky-500 shrink-0 mt-0.5" size={18} /> <span>Hem telefon hem bilgisayardan 7/24 dilediğiniz zaman hızlı erişim sağlayın</span>
                  </li>
                </ul>
        ...
        <section className="w-full h-[100dvh] min-h-[600px] snap-start snap-always bg-[var(--bg-secondary)] overflow-hidden pt-16 pb-4 flex flex-col border-b border-[var(--border-color)]">
           <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 flex flex-col h-full items-center">
             ...
             <div className="text-center w-full max-w-4xl mx-auto mt-6 lg:mt-8 mb-0 shrink-0 flex flex-col items-center">
               <h2 className="text-2xl sm:text-3xl md:text-4xl leading-tight font-extrabold text-[var(--text-main)] mb-3">Uygulamamızı Yakından Tanıyın</h2>
               <p className="text-sm sm:text-base md:text-lg text-[var(--text-muted)] font-medium max-w-3xl mx-auto px-4 leading-relaxed mb-2">
                 Can dostunuzun sağlık geçmişini, aşı takvimini ve klinik randevularını tek bir merkezden yönetin. Hayvansever topluluğumuzla sosyalleşirken, kliniğinizin günlük işleyişini kusursuz bir deneyimle organize edin.
               </p>
             </div>
             ...
             <div className="w-full flex-1 min-h-0 relative flex items-center justify-center">
               ...
               <div className="absolute right-0 md:right-1 lg:right-2 xl:right-4 top-1/2 -translate-y-1/2 z-40 hidden md:flex">
                 <a href="/features" className="group inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-[var(--color-vet-primary)] hover:bg-[var(--color-vet-secondary)] text-white font-semibold rounded-full transition-all shadow-sm hover:shadow-md">
                   Detaylı İncele <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                 </a>
               </div>
             </div>
        ...
        <section className="relative w-full h-[100dvh] snap-start snap-always bg-[var(--bg-main)] py-16 flex flex-col justify-center items-center overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 text-center">
            ...
            {/* Heading */}
            <h2 className="text-4xl md:text-5xl lg:text-[3.25rem] font-extrabold text-[var(--text-main)] leading-tight mb-8 -mt-8">
              Hemen İndirin <br className="hidden sm:block" /> Ayrıcalıkları Keşfedin
            </h2>
            
            {/* Description */}
            <div className="flex flex-col items-center justify-center w-full px-4 mb-12">
              <p className="text-xl md:text-2xl text-[var(--color-vet-primary)] font-bold mb-4">
                Evcil hayvan sahipleri için tamamen ücretsiz!
              </p>
              <p className="text-lg md:text-xl text-[var(--text-muted)] max-w-xl lg:max-w-2xl font-medium leading-relaxed">
                Klinikler için tüm premium yönetim özellikleri kısa bir süreliğine ücretsiz; <br/>
                <strong>kredi kartı gerekmeden</strong> anında deneyin.
              </p>
            </div>
