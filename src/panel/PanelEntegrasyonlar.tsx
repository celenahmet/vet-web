...
import {
  ...
  KeyRound, MessageCircle, RefreshCw, Send, ShieldCheck, Smartphone,
} from 'lucide-react';
...
export default function PanelEntegrasyonlar({ klinik, sahip }: { klinik: string; sahip: boolean }) {
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      const [e, s, i, sa, is, d, m, c] = await Promise.all([
        entegrasyonlariOku(klinik), entegrasyonSaglayicilariniOku(), iletisimIzinleriniOku(klinik),
        iletisimSablonlariniOku(), iletisimIsleriniOku(klinik), iletisimDogrulamalariniOku(klinik),
        musterileriOku(klinik), cevrimdisiMusterileriOku(klinik),
      ]);
      setEntegrasyonlar(e); setSaglayicilar(s); setIzinler(i); setSablonlar(sa);
      setIsler(is); setDogrulamalar(d); setMusteriler(m); setCevrimdisi(c);
  ...
  async function teknikAyariKaydet(tur: TeknikTur) {
    ...
    try {
      ...
      taslagiYama(tur, { sirlar: {} }); setBilgi(`${TUR_ADI[tur]} ayarları güvenli kasaya gönderildi ve doğrulama kuyruğuna alındı.`); await yukle();
  ...
  if (yukleniyor) return <Yukleniyor metin="Entegrasyon ve iletişim ayarları yükleniyor" />;
  ...
  return <section className="pnl-bolum">
    <header className="pnl-bolum-basi"><div><p className="pnl-aciklama">SMS, WhatsApp ve laboratuvar sağlayıcılarını; gizli kimlik bilgileri, sağlık durumu, izin ve spam korumalarıyla yönetin.</p></div><div className="pnl-basi-dugmeler"><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={() => setIzinAcik(true)}><ShieldCheck size={15} /> İletişim tercihi</button><button type="button" className="pnl-dugme pnl-dugme-olumlu" onClick={() => setGonderAcik(true)}><Send size={15} /> İleti gönder</button></div></header>
    {!sahip ? <div className="pnl-klinik-uyari"><KeyRound size={20} /><div><strong>Teknik kimlik bilgileri yalnız klinik sahibi tarafından değiştirilebilir</strong><p>Çalışanlar bağlantı ve ileti işlerinin durumunu görebilir; sağlayıcı sırlarını yazamaz.</p></div></div> : null}
    ...
    <div className="pnl-entegrasyon-listesi">{TURLER.map((tur) => {
