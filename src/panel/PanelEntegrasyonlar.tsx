...
export default function PanelEntegrasyonlar({ klinik, sahip, gorunum = 'technical', git }: {
  ...
  const [kurulumGorunumu, setKurulumGorunumu] = useState<'easy' | 'expert'>('easy');
  ...
  return <section className={`pnl-bolum pnl-yeni-modul pnl-yeni-modul-${gorunum}`}>
    <header className="pnl-bolum-basi pnl-yeni-modul-basi"><div><p className="pnl-aciklama">{gorunum === 'technical' ? 'Yalnız teknik kurulum: SMS, WhatsApp, laboratuvar/LIS-cihaz ve e-reçete sağlayıcılarının API, sağlık ve maskeli kimlik bilgilerini yönetin.' : 'Günlük klinik akışları: iletişim izinlerini yönetin, güvenli şablonları kuyruğa alın, teslimi izleyin ve ilgili operasyon ekranlarına geçin.'}</p></div>{gorunum === 'communications' ? <div className="pnl-basi-dugmeler"><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={() => setIzinAcik(true)}><ShieldCheck size={15} /> İletişim tercihi</button><button type="button" className="pnl-dugme pnl-dugme-olumlu" onClick={() => setGonderAcik(true)}><Send size={15} /> İleti gönder</button></div> : null}</header>
    ...
    {gorunum === 'technical' ? <section className="pnl-entegrasyon-rehberi" aria-labelledby="entegrasyon-ayarlari-baslik"><div><h2 id="entegrasyon-ayarlari-baslik">Entegrasyon ayarları</h2><p>{kurulumGorunumu === 'easy' ? 'Bağlamak istediğiniz sistemi seçin; panel eksik adımı ve bağlantı durumunu sade biçimde göstersin.' : 'Sağlayıcı, ortam, uç nokta, hesap/webhook alanları ve gizli anahtarları ayrıntılı olarak yönetin.'}</p><div className="pnl-segment pnl-entegrasyon-kurulum-sekmesi" aria-label="Entegrasyon kurulum görünümü"><button type="button" className={kurulumGorunumu === 'easy' ? 'pnl-segment-etkin' : ''} onClick={() => setKurulumGorunumu('easy')}>Kolay kurulum</button><button type="button" className={kurulumGorunumu === 'expert' ? 'pnl-segment-etkin' : ''} onClick={() => setKurulumGorunumu('expert')}>Uzman ayarları</button></div></div>{kurulumGorunumu === 'expert' ? <ul><li><strong>Genel ayarlar</strong><span>Sağlayıcı, ortam, API adresi, gönderici ve cihaz/LIS alanları</span></li><li><strong>Gizli bilgiler</strong><span>API key, token, parola ve webhook sırrı yalnız kasaya yazılır</span></li><li><strong>Bağlantı durumu</strong><span>Yapılandırma, sağlık kontrolü, sürüm ve son hata sınıfı</span></li></ul> : <ul><li><strong>1 · Sistemi seçin</strong><span>SMS, WhatsApp, laboratuvar veya e-reçete</span></li><li><strong>2 · Eksikleri tamamlayın</strong><span>Panel yalnız gerekli alanları adım adım açar</span></li><li><strong>3 · Doğrulayın</strong><span>Kaydetmek bağlantının hazır olduğu anlamına gelmez</span></li></ul>}</section> : null}
    ...
    {gorunum === 'technical' && kurulumGorunumu === 'easy' ? <div className="pnl-entegrasyon-kolay-grid">{TURLER.map((tur) => { const satir = entegrasyon(tur); const durum = satir?.status ?? 'not_configured'; return <article className="pnl-widget pnl-entegrasyon-kolay-karti" key={tur}><span className="pnl-widget-ikon">{tur === 'laboratory' ? <Beaker size={18} /> : tur === 'sms' ? <Smartphone size={18} /> : <MessageCircle size={18} />}</span><div><strong>{TUR_ADI[tur]}</strong><p>{TUR_ACIKLAMA[tur]}</p><span className={`pnl-durum pnl-durum-${durum}`}>{DURUM_ADI[durum]}</span></div><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={() => { setAcikTur(tur); setKurulumGorunumu('expert'); }}>{satir ? 'Ayarları gözden geçir' : 'Kuruluma başla'}</button></article>; })}</div> : null}
    {gorunum === 'technical' && kurulumGorunumu === 'expert' ? <div className="pnl-entegrasyon-listesi">{TURLER.map((tur) => {
