...
export default function PanelEntegrasyonlar({ klinik, sahip, gorunum = 'technical', git }: {
  ...
  const [ayarPenceresiAcik, setAyarPenceresiAcik] = useState(false);
  ...
  function saglayiciyiSec(tur: TeknikTur, kod: string) {
    ...
    taslagiYama(tur, {
      ...
      temelAdres: ayni
        ? mevcut?.base_url ?? ''
        : secili?.allowed_base_hosts[0] ? `https://${secili.allowed_base_hosts[0]}` : '',
  ...
  async function teknikAyariKaydet(tur: TeknikTur) {
    ...
    const temelAdres = t.temelAdres.trim() || null;
    if (temelAdres) {
      try {
        const adres = new URL(temelAdres);
        if (adres.protocol !== 'https:' || adres.username || adres.password
            || (adres.port && adres.port !== '443') || adres.search || adres.hash
            || !secili.allowed_base_hosts.includes(adres.hostname.toLowerCase())) {
          return setHata('API adresi yalnız doğrulanmış sağlayıcı alan adını kullanabilir.');
        }
      } catch {
        return setHata('API adresi geçerli bir HTTPS adresi olmalıdır.');
      }
    }
    ...
    try {
      await entegrasyonAyariniKaydet({ klinik, tur, saglayici: t.saglayici, ortam: t.ortam, temelAdres, genelAyarlar: t.ayarlar, sirlar: Object.fromEntries(Object.entries(t.sirlar).filter(([, deger]) => deger.trim())) });
  ...
  const sablonHazirMi = (sablon: IletisimSablonu) => entegrasyon(sablon.channel)?.status === 'ready';
  const hazirSablonSayisi = sablonlar.filter(sablonHazirMi).length;
  ...
  return <section className={`pnl-bolum pnl-yeni-modul pnl-yeni-modul-${gorunum}`}>
    <header className="pnl-bolum-basi pnl-yeni-modul-basi"><div><p className="pnl-aciklama">{gorunum === 'technical' ? 'SMS, WhatsApp, laboratuvar/LIS-cihaz ve e-reçete bağlantılarının durumunu izleyin. Hassas teknik ayarlar ayrı, odaklı pencerede yönetilir.' : 'Günlük klinik akışları: iletişim izinlerini yönetin, güvenli şablonları kuyruğa alın, teslimi izleyin ve ilgili operasyon ekranlarına geçin.'}</p></div>{gorunum === 'technical' ? <div className="pnl-basi-dugmeler"><button type="button" className="pnl-dugme pnl-dugme-olumlu" onClick={() => setAyarPenceresiAcik(true)}><KeyRound size={15} /> Entegrasyon ayarlarını aç</button></div> : <div className="pnl-basi-dugmeler"><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={() => setIzinAcik(true)}><ShieldCheck size={15} /> İletişim tercihi</button><button type="button" className="pnl-dugme pnl-dugme-olumlu" onClick={() => setGonderAcik(true)}><Send size={15} /> İleti gönder</button></div>}</header>
