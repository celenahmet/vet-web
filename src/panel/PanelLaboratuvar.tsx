...
export default function PanelLaboratuvar({ klinik }: { klinik: string }) {
  ...
  return <section className="pnl-bolum">
    <header className="pnl-bolum-basi"><div><p className="pnl-aciklama">İstem, cihaz sonucu, teknik kalite, sürüm geçmişi ve kaynaklı klinik karar desteğini tek izlenebilir akışta yönetin.</p></div><div className="pnl-basi-dugmeler"><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={() => setOcrAcik(true)}><Camera size={15} /> Cihaz ekranını oku</button><button type="button" className="pnl-dugme pnl-dugme-olumlu" onClick={() => setYeniAcik(true)}><Plus size={15} /> İstem oluştur</button></div></header>
    ...
    {istemler.length === 0 ? <Bos baslik="Henüz laboratuvar istemi yok" aciklama="Hasta ve panel seçerek ilk laboratuvar istemini oluşturun." /> : <div className="pnl-lab-listesi">{istemler.map((istem) => {
      ...
      return <article className="pnl-widget pnl-lab-karti" key={istem.id}><button type="button" className="pnl-lab-kart-basi" onClick={() => setAcikIstem(acik ? null : istem.id)}><span className="pnl-widget-ikon"><FlaskConical size={17} /></span><span><strong>{hastaAdi(istem.pet_id)} · {p ? DISIPLIN[p.discipline] : istem.test_name}</strong><small>{istem.provider_name} · {new Date(istem.created_at).toLocaleString('tr-TR')}</small></span><em className={`pnl-durum pnl-durum-${istem.status}`}>{DURUM[istem.status]}</em>{acik ? <ChevronUp size={17} /> : <ChevronDown size={17} />}</button>
      {acik ? <div className="pnl-lab-ayrinti"><div className="pnl-lab-meta"><span><b>Sistem</b>{istem.lab_system_type ? SISTEM[istem.lab_system_type] : '—'}</span><span><b>Numune</b>{istem.specimen || '—'}</span><span><b>Dış istem</b>{istem.external_request_id || '—'}</span><span><b>Sonuç sürümü</b>{istem.current_result_revision || 'Henüz yok'}</span></div>
