...
export default function PanelLaboratuvar({ klinik, sahip }: { klinik: string; sahip: boolean }) {
  ...
  const [cihazPaneliAcik, setCihazPaneliAcik] = useState(false);
  const [yeniAcik, setYeniAcik] = useState(false);
  const [ocrIcinIstem, setOcrIcinIstem] = useState(false);
  ...
  const aktifCihazlar = cihazlar.filter((satir) => satir.is_active);
  const ocrUyumluIstemler = acikIstemler.filter((istem) => panel(istem)?.expected_analytes.length);
  const seciliOcrIstemi = ocrUyumluIstemler.find((istem) => istem.id === ocrIstemi);
  const seciliOcrCihazi = aktifCihazlar.find((cihaz) => cihaz.id === ocrCihazi);
  const ocrCihazCakismasi = Boolean(seciliOcrIstemi?.device_id
    && seciliOcrIstemi.device_id !== seciliOcrCihazi?.id);
  const ocrHazir = Boolean(seciliOcrIstemi && seciliOcrCihazi && !ocrCihazCakismasi);
  ...
  async function istemOlustur(e: React.FormEvent) {
    ...
    try {
      const yeniIstem = await labIstemiOlustur({ klinik, hasta: yeni.hasta, saglayici: yeni.saglayici.trim(), panel: yeni.panel, sistem: yeni.sistem, cihaz: yeni.cihaz || null, numune: yeni.numune.trim() || null, disKimlik: yeni.disKimlik.trim() || null });
      ...
      if (ocrIcinIstem) {
        setOcrIcinIstem(false); setOcrIstemi(yeniIstem); setOcrAcik(true);
      }
    } catch (e) { setHata((e as Error).message); } finally { setBekliyor(false); }
  }

  function ocrPenceresiniAc() {
    const cihaz = seciliOcrCihazi ?? aktifCihazlar[0];
    const istem = seciliOcrIstemi ?? (ocrUyumluIstemler.length === 1 ? ocrUyumluIstemler[0] : undefined);
    if (cihaz) setOcrCihazi(cihaz.id);
    if (istem) {
      setOcrIstemi(istem.id);
      if (istem.device_id && aktifCihazlar.some((satir) => satir.id === istem.device_id)) setOcrCihazi(istem.device_id);
    }
    setHata(null); setOcrAcik(true);
  }

  function ocrIcinYeniIstemAc() {
    const cihaz = seciliOcrCihazi ?? aktifCihazlar[0];
    setOcrAcik(false); setOcrIcinIstem(true);
    setYeni((onceki) => ({ ...onceki, cihaz: cihaz?.id ?? '',
      saglayici: cihaz?.display_name ?? onceki.saglayici,
      sistem: cihaz?.lab_system_type ?? onceki.sistem }));
    setYeniAcik(true);
  }

  function yeniIstemPenceresiniKapat() {
    setYeniAcik(false);
    if (ocrIcinIstem) { setOcrIcinIstem(false); setOcrAcik(true); }
  }

  function cihazAyarlarinaGit() {
    setOcrAcik(false); setCihazPaneliAcik(true);
    requestAnimationFrame(() => document.getElementById('laboratuvar-cihazlari')
  ...
  async function fotografiOku(dosya: File) {
    ...
    try {
      ...
      const worker = await createWorker('eng', 1, {
        ...
        corePath: '/ocr/v7/tesseract-core-lstm.wasm.js',
        langPath: '/ocr/v7',
        logger: (olay) => {
  ...
  return <section className="pnl-bolum pnl-yeni-modul pnl-yeni-modul-operasyon">
    ...
    <LabCihazlari klinik={klinik} sahip={sahip} cihazlar={cihazlar} eslemeler={cihazEslemeleri} yenile={yukle}
      acik={cihazPaneliAcik} acikDegistir={setCihazPaneliAcik} />
    ...
    <Diyalog acik={yeniAcik} kapat={yeniIstemPenceresiniKapat} baslik={ocrIcinIstem ? 'OCR için laboratuvar istemi oluştur' : 'Yeni laboratuvar istemi'} aciklama="Hasta, cihaz, sistem türü ve panel seçimi sonuçların teknik kalite denetimini belirler."><form onSubmit={istemOlustur}><div className="pnl-alan"><label htmlFor="lab-hasta">Hasta</label><select id="lab-hasta" required value={yeni.hasta} onChange={(e) => setYeni({ ...yeni, hasta: e.target.value })}><option value="">Hasta seçin</option>{hastalar.map((hasta) => <option key={hasta.pet_id} value={hasta.pet_id}>{hasta.pet_name}</option>)}</select></div><div className="pnl-alan"><label htmlFor="lab-cihaz">Sonucu üreten cihaz profili</label><select id="lab-cihaz" required={yeni.sistem === 'in_house_analyzer' || ocrIcinIstem} value={yeni.cihaz} onChange={(e) => { const cihaz = cihazlar.find((satir) => satir.id === e.target.value); setYeni({ ...yeni, cihaz: e.target.value, ...(cihaz ? { saglayici: cihaz.display_name, sistem: cihaz.lab_system_type } : {}) }); }}><option value="">Cihaz seçin</option>{aktifCihazlar.map((cihaz) => <option key={cihaz.id} value={cihaz.id}>{cihaz.display_name} · {cihaz.manufacturer} {cihaz.model}</option>)}</select><span className="pnl-alan-ipucu">Cihaz seçimi OCR eşlemelerini ve sonuç kaynağını sabitler.</span></div><div className="pnl-alan"><label htmlFor="lab-sistem">Laboratuvar sistem türü</label><select id="lab-sistem" value={yeni.sistem} onChange={(e) => setYeni({ ...yeni, sistem: e.target.value as LabSistemTuru })}>{Object.entries(SISTEM).map(([kod, ad]) => <option key={kod} value={kod}>{ad}</option>)}</select></div><div className="pnl-alan"><label htmlFor="lab-panel">Test / panel</label><select id="lab-panel" required value={yeni.panel} onChange={(e) => setYeni({ ...yeni, panel: e.target.value })}><option value="">Panel seçin</option>{paneller.map((satir) => <option key={satir.code} value={satir.code}>{DISIPLIN[satir.discipline]} · {satir.code} · {satir.expected_analytes.length} analit</option>)}</select></div><div className="pnl-alan"><label htmlFor="lab-saglayici">Laboratuvar / cihaz adı</label><input id="lab-saglayici" required value={yeni.saglayici} onChange={(e) => setYeni({ ...yeni, saglayici: e.target.value })} /></div><div className="pnl-form-ikili"><div className="pnl-alan"><label htmlFor="lab-numune">Numune</label><input id="lab-numune" value={yeni.numune} onChange={(e) => setYeni({ ...yeni, numune: e.target.value })} placeholder="Serum, EDTA kan…" /></div><div className="pnl-alan"><label htmlFor="lab-dis">Sağlayıcı istem no.</label><input id="lab-dis" value={yeni.disKimlik} onChange={(e) => setYeni({ ...yeni, disKimlik: e.target.value })} /></div></div><div className="pnl-diyalog-eylem"><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={yeniIstemPenceresiniKapat}>Vazgeç</button><button type="submit" className="pnl-dugme pnl-dugme-olumlu" disabled={bekliyor || !yeni.hasta || !yeni.panel || !yeni.saglayici.trim() || ((yeni.sistem === 'in_house_analyzer' || ocrIcinIstem) && !yeni.cihaz)}>{bekliyor ? 'Oluşturuluyor…' : ocrIcinIstem ? 'İstemi oluştur ve OCR’a dön' : 'İstemi oluştur'}</button></div></form></Diyalog>
    ...
    <Diyalog acik={ocrAcik} kapat={() => { if (!bekliyor) { setOcrAcik(false); setOcrSatirlari([]); } }} baslik="Cihaz ekranından sonuç tara" aciklama="Fotoğraf yalnız bu tarayıcı sekmesinde OCR için kullanılır; sunucuya yüklenmez ve okuma tamamlanınca nesne adresi silinir.">
      <div className="pnl-form-ikili pnl-ocr-secimler">
        <div className="pnl-alan"><label htmlFor="ocr-cihaz">Sonucu üreten cihaz</label><select id="ocr-cihaz" value={ocrCihazi} disabled={aktifCihazlar.length === 0 || bekliyor} onChange={(e) => { setOcrCihazi(e.target.value); setOcrSatirlari([]); }}><option value="">Cihaz seçin</option>{aktifCihazlar.map((cihaz) => <option key={cihaz.id} value={cihaz.id}>{cihaz.display_name} · {cihaz.manufacturer} {cihaz.model}</option>)}</select></div>
        <div className="pnl-alan"><label htmlFor="ocr-istem">Açık laboratuvar istemi</label><select id="ocr-istem" value={ocrIstemi} disabled={ocrUyumluIstemler.length === 0 || bekliyor} onChange={(e) => { const istem = ocrUyumluIstemler.find((satir) => satir.id === e.target.value); setOcrIstemi(e.target.value); if (istem?.device_id && aktifCihazlar.some((cihaz) => cihaz.id === istem.device_id)) setOcrCihazi(istem.device_id); setOcrSatirlari([]); }}><option value="">Hasta ve istem seçin</option>{ocrUyumluIstemler.map((istem) => <option key={istem.id} value={istem.id}>{hastaAdi(istem.pet_id)} · {panel(istem)?.code} · {istem.provider_name}</option>)}</select></div>
      </div>
      <div className="pnl-ocr-onkosullar" aria-live="polite">
        {aktifCihazlar.length === 0 ? <div className="pnl-ocr-onkosul pnl-ocr-onkosul-eksik"><CircleAlert size={18} /><div><strong>Aktif cihaz profili gerekli</strong><p>OCR kod ve birim eşlemelerini kaynak cihaza göre uygular.</p></div>{sahip ? <button type="button" className="pnl-dugme pnl-dugme-sade" onClick={cihazAyarlarinaGit}>Cihaz profili ekle</button> : <span>Klinik sahibinden cihaz profili eklemesini isteyin.</span>}</div> : null}
        {ocrUyumluIstemler.length === 0 ? <div className="pnl-ocr-onkosul pnl-ocr-onkosul-eksik"><CircleAlert size={18} /><div><strong>Açık laboratuvar istemi gerekli</strong><p>Hasta ve panel bağı olmadan OCR sonucu sağlık kaydına yazılmaz.</p></div><button type="button" className="pnl-dugme pnl-dugme-sade" disabled={aktifCihazlar.length === 0} onClick={ocrIcinYeniIstemAc}>Hasta ve istem seç</button></div> : null}
        {ocrCihazCakismasi ? <div className="pnl-ocr-onkosul pnl-ocr-onkosul-eksik"><CircleAlert size={18} /><div><strong>İstem farklı cihaza bağlı</strong><p>İstemin cihazını seçin veya bu cihaz için yeni istem oluşturun; sonuçlar sessizce karıştırılmaz.</p></div><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={ocrIcinYeniIstemAc}>Yeni istem oluştur</button></div> : null}
        {ocrHazir ? <div className="pnl-ocr-onkosul pnl-ocr-onkosul-hazir"><Check size={18} /><div><strong>OCR için hazır</strong><p>{hastaAdi(seciliOcrIstemi!.pet_id)} · {panel(seciliOcrIstemi!)?.code} · {seciliOcrCihazi!.display_name}</p></div></div> : null}
      </div>
      <label className={ocrHazir && !bekliyor ? 'pnl-dosya-alani' : 'pnl-dosya-alani pnl-dosya-kapali'} aria-disabled={!ocrHazir || bekliyor}><Camera size={24} /><strong>{ocrHazir ? 'Cihaz ekranı fotoğrafını seç veya çek' : 'Önce cihaz ve hasta istemini tamamlayın'}</strong><span>JPEG, PNG veya WebP · en fazla 12 MB · fotoğraf saklanmaz</span><input ref={dosyaAlani} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" disabled={!ocrHazir || bekliyor} onChange={(e) => { const dosya = e.target.files?.[0]; if (dosya) void fotografiOku(dosya); }} /></label>
      {ocrIlerleme != null ? <div className="pnl-ilerleme"><span style={{ width: `${ocrIlerleme}%` }} /><p>{ocrDurum} · %{ocrIlerleme}</p></div> : null}
      {ocrSatirlari.length ? <><div className="pnl-gizlilik-notu"><ShieldCheck size={17} /><span>Fotoğraf silindi; yalnız aşağıdaki geçici OCR taslağı bellekte. Kayda yalnız veterinerin doğruladığı analitler gider.</span></div><p className="pnl-ocr-ozet">Aynı {ocrSayilari.same} · yeni {ocrSayilari.new} · çakışan {ocrSayilari.conflict} · okunamayan {ocrSayilari.unreadable} · mevcut korunan {ocrSayilari.existing_only}</p>{ocrSayilari.conflict ? <p className="pnl-ocr-politika">Çakışmalarda mevcut klinik değeri varsayılan korunur. Yalnız açıkça “Tarananı kullan” seçtiğiniz satırlar değişir.</p> : null}<div className="pnl-ocr-satirlar">{ocrSatirlari.map((satir) => <div key={satir.code} className={`pnl-ocr-satir pnl-ocr-${satir.status}`}><header><strong>{satir.code}</strong><span>{OCR_DURUM[satir.status]}</span></header>{satir.status === 'conflict' ? <><p>Mevcut: {analitDegeri(satir.existing)}</p><p>Taranan: {ocrDegeri(satir)}</p><div className="pnl-segment"><button type="button" className={satir.choice === 'existing' ? 'pnl-segment-etkin' : ''} onClick={() => ocrSec(satir.code, 'existing')}>Mevcut değeri koru</button><button type="button" className={satir.choice === 'scanned' ? 'pnl-segment-etkin' : ''} onClick={() => ocrSec(satir.code, 'scanned')}>Tarananı kullan</button></div></> : satir.status === 'unreadable' && !satir.existing ? <div className="pnl-alan"><label htmlFor={`ocr-${satir.code}`}>Okunamayan değer (??)</label><input id={`ocr-${satir.code}`} inputMode="decimal" placeholder="Cihaz ekranından doğrulayıp girin" onChange={(e) => okunamayanDegeriYaz(satir.code, e.target.value)} /></div> : <p>{satir.choice === 'existing' ? analitDegeri(satir.existing) : ocrDegeri(satir)}</p>}</div>)}</div>{cozulmemis ? <p className="pnl-ocr-politika">{cozulmemis} yeni değer okunamadı. Cihaz ekranından doğrulayıp sayısal değeri girmeden sonuç kaydedilemez.</p> : !degisiklikVar ? <p className="pnl-widget-not">Tarama mevcut sonuçla aynı; gereksiz yeni revizyon oluşturulmayacak.</p> : null}<div className="pnl-diyalog-eylem"><button type="button" className="pnl-dugme pnl-dugme-sade" onClick={() => setOcrSatirlari([])}>Yeniden tara</button><button type="button" className="pnl-dugme pnl-dugme-olumlu" disabled={bekliyor || cozulmemis > 0 || !degisiklikVar} onClick={() => void ocrSonucunuKaydet()}>Doğrulanan sonucu kaydet</button></div></> : null}
    </Diyalog>
  </section>;
}
