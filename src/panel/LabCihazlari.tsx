  ...
  return <section className="pnl-widget">
    ...
    {acik ? <div className="pnl-widget-govde"><p className="pnl-widget-not">Her istem ve sonuç revizyonu kaynak cihaza sabitlenir. Aynı isteme farklı cihaz sonucu yazılamaz; bu durumda yeni istem açılır.</p>
      ...
      {sahip ? <div className="pnl-cihaz-formlari"><form onSubmit={cihazKaydet}><h4><Plus size={15} /> Cihaz profili ekle</h4>
        ...
        <button className="pnl-dugme pnl-dugme-olumlu" disabled={bekliyor || form.disiplinler.length === 0}>Cihazı kaydet</button></form>
        <form onSubmit={eslemeKaydet}><h4>Analit eşlemesi ekle</h4><p className="pnl-widget-not">Ham kodu kanonik koda bağlar; birim dönüşümü varsa katsayı kayıt altında kalır.</p>
          ...
          <button className="pnl-dugme pnl-dugme-olumlu" disabled={bekliyor || !esleme.cihaz}>Eşlemeyi kaydet</button></form></div> : <p className="pnl-dipnot">Cihaz profili ve eşleme ayarlarını yalnız klinik sahibi değiştirebilir.</p>}
    </div> : null}
  </section>;
}
