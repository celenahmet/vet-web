...
const newSection = `{/* 5. SAĞ ALT: Özellikler ve Logo */}
            <div className="absolute bottom-8 right-8 z-20 hidden md:flex flex-col w-[320px] lg:w-[350px] pointer-events-none">
              <AnimatePresence mode="wait">
                ...
                >
                  {/* Arka Plan ve İçe Göçük (Cutout) - CSS Multiple Masks */}
                  <div 
                    className="absolute inset-0 bg-white/15 dark:bg-black/20 backdrop-blur-[40px] rounded-[45px] shadow-2xl"
                    style={{ 
                      WebkitMaskImage: \`
                        radial-gradient(circle at 36px calc(100% - 36px), transparent 52px, black 53px),
                        radial-gradient(circle at 0px calc(100% - 74px), transparent 16px, black 17px),
                        radial-gradient(circle at 74px 100%, transparent 16px, black 17px)
                      \`, 
...
console.log('Successfully applied multiple CSS masks for tight filleted cutout.');
