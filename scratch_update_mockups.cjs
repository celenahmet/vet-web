...
const oldMockups = `                    {/* Incoming SMS */}
                    <motion.div
                      initial={{ opacity: 0, x: -10 }}
                      whileInView={{ opacity: 1, x: 0 }}
                      viewport={{ once: true }}
                      className="bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 p-2.5 rounded-xl rounded-tl-sm shadow-sm max-w-[90%]"
                    >
                      <div className="flex items-center gap-1.5 text-[10px] text-slate-400 mb-1">
                        <Smartphone size={10} /> SMS Bildirimi
                      </div>
                      <p className="text-[12px] leading-tight text-slate-700 dark:text-slate-300">
                        Sayın hasta sahibimiz, Leo'nun laboratuvar sonuçları çıkmıştır. Hekiminiz değerlendirdikten sonra size bilgi verecektir.
                      </p>
                    </motion.div>
                    ...
                    <motion.div
                      ...
                      className="bg-[#e6f4ea] dark:bg-[#132c1e] border border-[#c4e3d0] dark:border-green-800/30 p-2.5 rounded-xl rounded-tr-sm shadow-sm max-w-[90%] ml-auto"
                    >
                      <div className="flex justify-end items-center gap-1.5 text-[10px] text-green-600/80 dark:text-green-400/70 mb-1">
                        Veterito Klinik - WhatsApp <CheckCircle2 size={10} />
                      </div>
                      <p className="text-[12px] leading-tight text-green-900 dark:text-green-300">
                        Merhaba, yarın saat 14:00'teki muayene randevunuzu hatırlatmak isteriz. 🐾
                      </p>
                    </motion.div>`;

const newMockups = `                    {/* Incoming SMS - Lab */}
                    <motion.div
                      initial={{ opacity: 0, x: -10 }}
                      whileInView={{ opacity: 1, x: 0 }}
                      viewport={{ once: true }}
